...
function! FilterGameList(arg_lead, cmd_line_unused, cursor_pos_unused)
    return filter(keys(g:game_default_games), 'stridx(v:val, a:arg_lead) == 0')
endfun

command! -nargs=* -complete=customlist,FilterGameList Game call RunGame(<f-args>)
